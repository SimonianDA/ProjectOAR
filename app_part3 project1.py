...
df = pd.read_csv(r'C:\Users\123\Downloads\hhru_backfront3.csv')
...
def pagecontent(pathname):
    if pathname == "/page1":
        ...
        html.Div([
            html.H1("Показатели зарплат по профессии бэкенд и фронтэнд разработчик"),


            html.P(
                "Анализ средних зарплат за 2021 и 2022."
        ...
        html.Div(
            dcc.Graph(figure=px.scatter_3d(df, x = 'name', y = 'Средняя зарплата', z = 'area_name', width=1500, height=1500)),
            # style = {'width': '70%', 'float': 'right', 'display': 'inline-block'}
        ),

        html.Div(
            dcc.Graph(figure=px.bar(df, x = 'area_name', y = 'name', width=1500, height=1000)),
            # style = {'width': '70%', 'float': 'right', 'display': 'inline-block'}
        ),
        
        html.Div(
            dcc.Graph(figure=px.scatter(df, x = 'Средняя зарплата', y = 'salary_currency', )),
            # style = {'width': '70%', 'float': 'right', 'display': 'inline-block'}
        ),
